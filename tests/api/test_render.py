...
class TestComponentRenderView:
    def test_render_returns_200_ok_and_html(self, registry_with_demo_components):
        with patch("dj_design_system.api.views.component_registry", registry_with_demo_components):
    ...
    def test_render_400_bad_request_invalid_json(self):
        factory = RequestFactory()
        request = factory.post("/api/render/", data="not json", content_type="application/json")
        view = ComponentRenderView.as_view()
        response = view(request)
    ...
    def test_render_400_bad_request_missing_component(self):
        factory = RequestFactory()
        payload = {"params": {}}
        request = factory.post("/api/render/", data=json.dumps(payload), content_type="application/json")
        view = ComponentRenderView.as_view()
        response = view(request)
        assert response.status_code == 400
        data = json.loads(response.content)
    ...
    def test_render_404_not_found_invalid_component(self, registry_with_demo_components):
        with patch("dj_design_system.api.views.component_registry", registry_with_demo_components):
            factory = RequestFactory()
            payload = {
    ...
    def test_render_400_on_component_render_error(self, registry_with_demo_components):
        ...
        with patch("dj_design_system.api.views.component_registry", registry_with_demo_components):
            with patch("dj_design_system.api.views.render_component") as mock_render:
                mock_render.side_effect = TypeError("missing 1 required positional argument: 'content'")
                ...
                data = json.loads(response.content)
                assert "error" in data
                assert "Failed to render component. Please check your parameters." in data["error"]
